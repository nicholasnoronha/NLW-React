...
import { generateRandomId } from "../pages/NewRoom";
...
export function Room() {
  ...
  async function handleSendQuestion(event: FormEvent) {
    ...
    const questionRef = ref(
      database,
      "rooms/" + roomId + "/questions/" + (await generateRandomId(20))
    );
  ...
  async function handleLikeQuestion(questionId: string, likeId?: string) {
    if (likeId) {
      ...
      const likeRef = ref(
        database,
        `rooms/${roomId}/questions/${questionId}/likes/${randomId}`
      );
      await set(likeRef, { authorId: user?.id });
    }
  }
